			...
			foreach($posts as $key => $post) {
				...
				$format         = get_post_format();
					...
					if ($thumbs != 'none') {
						...
						if ($meta == 'yes') {
								// mini post meta
									$output .= '<time datetime="'.get_the_time('Y-m-d\TH:i:s', $post->ID).'"> <span>' .get_the_time('d.m.Y', $post->ID). '</span></time>';
							}
							$output .= '<span class="list-title"><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'">';
