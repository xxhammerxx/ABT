...
<form method="post" action="<?php echo esc_url( add_query_arg( 'tab', 'google_calendar' ) ) ?>"
    ...
    <div class="form-group">
        ...
        <input id="ab_settings_google_client_id" class="form-control" type="text" name="ab_settings_google_client_id"
               value="<?php echo get_option( 'ab_settings_google_client_id' ) ?>"/>
    </div>
    <div class="form-group">
        ...
        <input id="ab_settings_google_client_secret" class="form-control" type="text"
               name="ab_settings_google_client_secret"
               value="<?php echo get_option( 'ab_settings_google_client_secret' ) ?>"/>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <?php \Bookly\Lib\Utils\Common::optionToggle( 'ab_settings_google_two_way_sync', array( array( '0', __( 'Disabled', 'bookly' ) ), array( '1', __( 'Enabled', 'bookly' ) ) ) ) ?>
    </div>
    <div class="form-group">
        ...
        <select id="ab_settings_google_limit_events" class="form-control" name="ab_settings_google_limit_events">
            <?php foreach ( array( __( 'Disabled', 'bookly' ) => '0', 25 => 25, 50 => 50, 100 => 100, 250 => 250, 500 => 500, 1000 => 1000, 2500 => 2500 ) as $text => $limit ) : ?>
                <option
                    value="<?php echo $limit ?>" <?php selected( get_option( 'ab_settings_google_limit_events' ), $limit ) ?> ><?php echo $text ?></option>
            <?php endforeach ?>
        </select>
    </div>
    <div class="form-group">
        <label for="ab_settings_google_event_title"><?php _e( 'Template for event title', 'bookly' ) ?></label>
        <p class="help-block"><?php _e( 'Configure what information should be placed in the title of Google Calendar event. Available codes are [[SERVICE_NAME]], [[STAFF_NAME]] and [[CLIENT_NAMES]].', 'bookly' ) ?></p>
        <input id="ab_settings_google_event_title" class="form-control" type="text"
               name="ab_settings_google_event_title"
               value="<?php echo esc_attr( get_option( 'ab_settings_google_event_title', '[[SERVICE_NAME]]' ) ) ?>">
    </div>
