...
class Controller extends Lib\Base\Controller
{
    ...
    public function index()
    {
        ...
        wp_localize_script( 'ab-staff.js', 'BooklyL10n',  array(
            ...
            'no_locations_selected' => __( 'No locations selected', 'bookly' ),
    ...
    public function executeUpdateStaff()
    {
        ...
        if ( class_exists( '\BooklyLocations\Lib\Plugin' ) ) {
            $location_ids = $this->getParameter( 'location_ids', array() );
            $staff_id     = intval( $this->getParameter( 'id' ) );
            if ( empty( $location_ids ) ) {
                \BooklyLocations\Lib\Entities\StaffLocation::query()->delete()->where( 'staff_id', $staff_id )->execute();
            } else {
                \BooklyLocations\Lib\Entities\StaffLocation::query()->delete()->where( 'staff_id', $staff_id )->whereNotIn( 'location_id', $location_ids )->execute();
                $staff_locations_exists = \BooklyLocations\Lib\Entities\StaffLocation::query()->select( 'location_id' )->where( 'staff_id', $staff_id )->fetchArray();
                $staff_locations        = array();
                foreach ( $staff_locations_exists as $location ) {
                    $staff_locations[] = $location['location_id'];
                }
                foreach ( $location_ids as $location_id ) {
                    if ( ! in_array( $location_id, $staff_locations ) ) {
                        $staff_location = new \BooklyLocations\Lib\Entities\StaffLocation();
                        $staff_location->set( 'staff_id', $staff_id );
                        $staff_location->set( 'location_id', $location_id );
                        $staff_location->save();
                    }
                }
            }
    ...
    protected function getHolidays( $staff_id )
    {
        ...
        foreach ( $collection as $holiday ) {
            ...
            if ( ! $holiday['repeat_event'] ) {
                $holidays[ $holiday['id'] ]['y'] = intval( $Y );
            }
        }
